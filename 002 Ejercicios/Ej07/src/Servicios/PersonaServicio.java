...
public class PersonaServicio {
    ...
    public Persona crearPersona() {
        ...
        if(per.getSexo()!='M' && per.getSexo()!='F' && per.getSexo()!='O') System.out.println("Sexo incorrecto");
    ...
    public int calcularIMC(Persona per){
        ...
        if(per.getPeso()/(per.getAltura()*(per.getAltura()))<20){
            valor = -1;
        } else if (per.getPeso()/(per.getAltura()*(per.getAltura()))>25){
            valor = 1;
        }
        
        System.out.println("Su IMC es " + per.getPeso()/(per.getAltura()*(per.getAltura())));
        
        return valor;
    }
    
    public void  esMayorDeEdad(Persona per){
        if(per.getEdad()<18) System.out.println("Eres menor de edad");
